...
const softDeleteUser = (req, res) => {
  const userId = req.params.id;
  db.run('UPDATE users SET is_deleted = 1 WHERE user_id = ?', [userId], function (err) {
    if (err) {
      return res.status(500).json({ error: err.message });
    }
    if (this.changes === 0) {
...
const getMedicationById = (req, res) => {
  ...
  db.get('SELECT * FROM medication WHERE med_id = ? AND is_deleted = 0', [medicationId], (err, row) => {
    if (err) {
      return res.status(500).json({ error: err.message });
    }
    if (!row) {
...
const softDeleteMedication = (req, res) => {
  const medicationId = req.params.id;
  db.run('UPDATE medication SET is_deleted = 1 WHERE med_id = ?', [medicationId], function (err) {
    if (err) {
      return res.status(500).json({ error: err.message });
    }
    if (this.changes === 0) {
...
const hardDeleteMedication = (req, res) => {
  const medicationId = req.params.id;
  db.run('DELETE FROM medication WHERE med_id = ?', [medicationId], function (err) {
    if (err) {
      return res.status(500).json({ error: err.message });
    }
    if (this.changes === 0) {
...
const getCustomerById = (req, res) => {
  ...
  db.get('SELECT * FROM customers WHERE customer_id = ? AND is_deleted = 0', [customerId], (err, row) => {
    if (err) {
      return res.status(500).json({ error: err.message });
    }
    if (!row) {
...
const softDeleteCustomer = (req, res) => {
  const customerId = req.params.id;
  db.run('UPDATE customers SET is_deleted = 1 WHERE customer_id = ?', [customerId], function (err) {
    if (err) {
      return res.status(500).json({ error: err.message });
    }
    if (this.changes === 0) {
...
const hardDeleteCustomer = (req, res) => {
  const customerId = req.params.id;
  db.run('DELETE FROM customers WHERE customer_id = ?', [customerId], function (err) {
    if (err) {
      return res.status(500).json({ error: err.message });
    }
    if (this.changes === 0) {
